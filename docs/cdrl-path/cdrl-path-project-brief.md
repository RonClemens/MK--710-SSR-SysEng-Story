...
| Developmental lineage schema | **Added 2026-08-14** (see DECISIONS.md #62-65): new `derived_from: string[]` node field — directional, backward-pointing, a curated subset of `influenced_by` representing strict Vee-model flow-down derivation (distinct from the looser `influences`/`influenced_by`). Surfaced as a "Developmental lineage" section in the station detail panel (Derives from / Flows into), and as a toggleable, off-by-default arrow overlay on the subway map (solid dark, arrowhead, distinct from the existing dashed-gray relationship tracks). Matrix view has no lineage indicator yet — open item below, not silently scoped out. Same first-pass/unconfirmed caveat as `influences`/`influenced_by`. |
...
- Matrix view has no developmental-lineage indicator yet — the arrow overlay is subway-map-only so far (see View modes / Developmental lineage schema rows above)
