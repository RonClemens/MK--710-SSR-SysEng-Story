...
| Relationship-edge rendering | **Added 2026-08-11**, not originally scoped to any phase (see DECISIONS.md #6). **Revised 2026-08-13** (see DECISIONS.md #33-35): every `influences`/`influenced_by` pair is bucketed into a shared "handoff hub" by domain pair + nearest half-ring, sitting radially between two SETR rings — depicting the working session/handoff two domains need to reach maturity by roughly that point — rather than the earlier PRR-column bullseye (#7) or raw point-to-point diagonals. |
