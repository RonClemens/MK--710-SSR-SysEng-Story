...
| Relationship-edge rendering | **Added 2026-08-11**, not originally scoped to any phase (see DECISIONS.md #6). **Revised 2026-08-13** (see DECISIONS.md #33-35), then **superseded 2026-08-14** (see DECISIONS.md #51): the domain-pair "handoff hub" diamonds from the 08-13 revision are removed entirely — `influences`/`influenced_by` currently has no dedicated map visual. Every SETR ring's label (and any transfer hub sitting on that ring) is now an invisible click target opening "CDRLs required at this event" instead. Flagged as an open trade-off, not a settled decision: if per-relationship visualization still matters on its own terms, it needs to come back as an explicit future ask. |
