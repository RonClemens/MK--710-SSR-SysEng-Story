...
import { PLANNING_CDRL_CATALOG } from "../../../methodology/guidance/planningGuidance";
...
export interface PlanningDeliverableValues {
  title: string;
  level: SpecLevel;
  cdrlType: string;
...
interface Props {
  initial: PlanningDeliverableValues;
  subsystems: LogicalSubsystem[];
  cis: ConfigurationItem[];
  onSubmit: (values: PlanningDeliverableValues) => Promise<void>;
  onCancel: () => void;
  submitLabel?: string;
}

export function PlanningDeliverableForm({ initial, subsystems, cis, onSubmit, onCancel, submitLabel = "Save" }: Props) {
  ...
  const catalog = PLANNING_CDRL_CATALOG[level];

  function handleLevelChange(next: SpecLevel) {
    setLevel(next);
    const stillValid = PLANNING_CDRL_CATALOG[next].some((c) => c.name === cdrlType);
    if (!stillValid) {
      const first = PLANNING_CDRL_CATALOG[next][0];
